...
// added by Steven Volocyk
void drawDisk(double inner, double outer, int slices, int loops) {
...
// added by Steven Volocyk
void generateLogWall(double length, double width, double height, bool frontOrBack, bool useTexture, int type){
	double verticalOffset = 4.7;
	double subSectionLength = 40;
	for (int i = 0; i <= height; ++i){
		...
		drawCylinder(width, width, length, 5, 1);
		//glBindTexture(GL_TEXTURE_2D, 0);
		if (useTexture){
			fetchTexture("stump.jpg", false, false);
		}
		drawDisk(0.0, 2.5, 5, 1);
		glTranslated(0.0, 0.0, length);
		drawDisk(0.0, 2.5, 5, 1);
		glTranslated(0.0, 0.0, -length);

		glTranslated(0.0, verticalOffset, 0.0);
		//verticalOffset += 3.0;
	}
	double offset = 3.3;
	if (frontOrBack == true){
		for (int i = 0; i < height - 2; ++i) {
			...
			drawCylinder(width, width, subSectionLength, 5, 1);
			if (useTexture){
				fetchTexture("stump.jpg", false, false);
			}
			drawDisk(0.0, 2.5, 5, 1);
			glTranslated(0.0, 0.0, subSectionLength);
			drawDisk(0.0, 2.5, 5, 1);
			glTranslated(0.0, 0.0, -subSectionLength);
			glTranslated(0.0, verticalOffset, 0.0);
			subSectionLength -= 14.0;
			offset += 2.6;
		} 
	}
}
// added by Steven Volocyk
void generateLogFence(double length, double width, double height, int type) {
	double Offset = 1.0;
	double subSectionLength = 40;
	//long piece1
	if (type == 1) {
	...
	drawCylinder(width, width, length * 3, 5, 1);
	...
	//draw cross sections.
	for (double i = 0.0; i <= 3*length; ++i){
		drawCylinder(width/2, width/2, length, 5, 1);
